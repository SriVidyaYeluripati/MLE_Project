...
train_one() {                       # "<tag> <seed> <exp>"
  ...
  env LQ_WEIGHTS=w_$tag$s.npz LQ_BOMB_COST=0 LQ_CRATE_VALUE=0.02 $sw \
    ...
    --train 1 --scenario classic --n-rounds 600 --seed $((5000+s)) >/dev/null 2>&1 \
...
eval_one() {                        # "<tag> <seed> <exp>"
  ...
  case "$tag" in g80) sw="LQ_GAMMA=0.80";; g60) sw="LQ_GAMMA=0.60";;
                 hherd) sw="LQ_HERD=1";; esac
  local m=.exp_done/eval_$tag$s
  [ -f "$m" ] && [ -s "results/$exp/$tag$s.json" ] && { echo "skip  eval $tag$s"; return; }
  env LQ_WEIGHTS=w_$tag$s.npz $sw python main.py play --no-gui \
    ...
    --scenario classic --n-rounds 300 --save-stats results/$exp/$tag$s.json >/dev/null 2>&1 \
...
case "${1:-both}" in
  gamma) phase GAMMA jobs_gamma; python3 exp_report.py gamma ;;
  herd)  phase HERD  jobs_herd;  python3 exp_report.py herd ;;
  both)  phase GAMMA jobs_gamma; python3 exp_report.py gamma
         phase HERD  jobs_herd;  python3 exp_report.py herd ;;
  *) echo "usage: bash run_experiments.sh [gamma|herd|both]"; exit 1 ;;
esac
